...
function hosts_update() {
        cp /etc/hosts /etc/hosts.${install_time}
        sed -i "/.*${linux_hostname}.*/d" /etc/hosts
        ip_address=$(hostname -I | awk '{print $1}')
        echo "$ip_address $linux_hostname" >>etc/hosts
...
EOF
        echo "sleeping 300"
        sleep 300
        if [ -f "${oracle_home_dir}/root.sh" ]; then
...
function dbca_rsp_update() {
        dbca_file=$(find ${soft_path} -name dbca.rsp)
        echo $dbca_file
        sed -i 's/GDBNAME = "orcl11g.us.oracle.com".*/GDBNAME = "'${db_name}'"/g' $dbca_file
        sed -i 's/SID = "orcl11g".*/SID = "'${db_name}'"/g' $dbca_file
        sed -i 's/#SYSPASSWORD = "password".*/SYSPASSWORD = "oracle"/g' $dbca_file
        sed -i 's/#SYSTEMPASSWORD = "password".*/SYSTEMPASSWORD = "oracle"/g' $dbca_file
        sed -i 's/#EMCONFIGURATION = "NONE"/EMCONFIGURATION = "NONE"/g' $dbca_file
        sed -i 's/#SYSMANPASSWORD = "password".*/SYSMANPASSWORD = "oracle"/g' $dbca_file
        sed -i 's/#DBSNMPPASSWORD = "password".*/DBSNMPPASSWORD = "oracle"/g' $dbca_file
        sed -i 's/#CHARACTERSET = "US7ASCII".*/CHARACTERSET = "ZHS16GBK"/g' $dbca_file
        sed -i 's/#MEMORYPERCENTAGE = "40".*/MEMORYPERCENTAGE = "70"/g' $dbca_file
        # 模板文件中修改为库名为想要的库名
